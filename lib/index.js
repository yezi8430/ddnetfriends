...
async function ddlist(ctx) {
  ...
  const timeout = 1e4;
  const maxRetries = 3;
  const fetchWithRetry = /* @__PURE__ */ __name(async (url) => {
  ...
  const raceToSuccess = /* @__PURE__ */ __name((promises) => {
...
async function getimage(nickname1, warband, emoji) {
  ...
  const processColorBody = /* @__PURE__ */ __name((colorBody) => {
    ...
    let h2 = parseInt(hex.substring(0, 2), 16);
    let s = parseInt(hex.substring(2, 4), 16);
    let l = parseInt(hex.substring(4, 6), 16);
    h2 = Math.round(h2 / 255 * 360);
...
async function apply(ctx, Config2, session) {
  ...
  if (ctx.config.Special_Attention === true) {
    ctx.setInterval(async () => {
      try {
          ...
          if (existingRecord) {
            ...
            if (existingRecord.last_sent_time) {
              ...
              if (isNaN(lastSentTime.getTime())) {
                lastSentTime = /* @__PURE__ */ new Date(existingRecord.last_sent_time.replace(" ", "T") + "Z");
              }
            } else {
              lastSentTime = /* @__PURE__ */ new Date(0);
            }
            if (now.getTime() - lastSentTime.getTime() < ctx.config.Special_Attention_Interval_time * 60 * 60 * 1e3) {
              continue;
            }
          }
        ...
        const cleanupTime = new Date(now.getTime() - ctx.config.Special_Attention_Interval_time * 60 * 60 * 1e3);
  ...
  async function handleImageOutput(ctx2, session2, allfriends2, browser, config2) {
    let context, page;
    try {
      const { enablewarband, enableemoji } = config2;
      const gethtml = await getimage(allfriends2, enablewarband, enableemoji);
...
`).join("");
  ...
  ctx.command("dd在线").subcommand("dd删除好友").action(async ({ session: session2 }) => {
    ...
    try {
      ...
      const friendNames = content.replace(/，/g, ",").split(",").map((name2) => name2.trim());
      const validFriendNames = friendNames.filter((name2) => name2 !== "");
      if (validFriendNames.length === 0) {
        await sendMessage(session2, "未输入有效的好友名称，操作已取消。");
        return;
      }
  ...
  ctx.command("dd在线").subcommand("dd添加好友").action(async ({ session: session2 }) => {
    await sendMessage(session2, "请在游戏中点击皮肤目录,并返回上一级菜单,双击settings_ddnet.cfg,用笔记本或记事本打开,复制里面所有的内容粘贴过来(如果只添加单个好友请直接输入好友名)\n输入:取消\n可以取消输入");
    let content = await session2.prompt();
