...
async function apply(ctx, Config2, session) {
  ...
  if (ctx.config.Special_Attention === true) {
    const lastSentTimes = /* @__PURE__ */ new Map();
    ctx.setInterval(async () => {
      try {
        ...
        const sentUsers = /* @__PURE__ */ new Set();
        const now = Date.now();
        for (const item of Special_Attention_online) {
          ...
          if (lastSentTime && now - lastSentTime < ctx.config.Special_Attention_Interval_time * 60 * 60 * 1e3) {
            continue;
          }
          for (const data of groupData) {
            try {
              if (ddnetUserId === data.user_id && !sentUsers.has(data.user_id)) {
                ...
                sentUsers.add(data.user_id);
                lastSentTimes.set(friendname, now);
                break;
              }
            } catch (sendError) {
  ...
  ctx.command("dd在线").action(async ({ session: session2 }) => {
    if (Config2?.useimage === true) {
      ...
      try {
        ...
        if (backlist.length === 0) {
          session2.send("未找到相关数据,你似乎还没有导入好友列表");
          return;
        }
        ...
        const warband = Config2?.enablewarband === true;
        const emoji = Config2?.enableemoji === true;
        const colorbodyconfig = Config2?.colorbodyconfig === true;
        ...
        await Promise.race([
          page.waitForNetworkIdle({ idleTime: 500, timeout: 5e3 }),
        ...
        await session2.send(import_koishi.h.image(image, "image/png"));
      } catch (error) {
        ctx.logger.error("Error during operation:", error);
        session2.send("操作过程中发生错误，请稍后再试");
      } finally {
        if (page) {
      ...
      const htmldata = await ddlist(ctx);
      let newclientInfoArray2 = updateserverinfo(htmldata);
      const qqid = session2.userId;
      let backlist = await getlist(ctx, qqid);
      if (backlist.length === 0) {
        session2.send("未找到相关数据,你似乎还没有导入好友列表");
        ...
        let allfriends2 = checkAndPrintFriendsnew(newclientInfoArray2, backlist);
        if (allfriends2.length === 0) {
          session2.send("无在线好友");
          return;
        } else {
          let resultString = allfriends2.map((friend) => `${friend.friendname}
`).join("");
          session2.send((0, import_koishi.h)("at", { id: session2.userId }) + "你的在线好友:\n" + resultString);
          return;
        }
      }
    }
  });
  ctx.command("dd删除好友").action(async ({ session: session2 }) => {
    const userid = session2.userId;
    session2.send("输入好友列表，如 123,234,abc,支持单个或批量");
    if (session2.content !== "") {
      const dispose = ctx.on("message", async (session3) => {
        const str = session3.content;
        ...
        if (deletesum !== 0) {
          session3.send("已删除" + deletesum + "个好友");
          dispose();
        } else {
          session3.send("删除" + deletesum + "个好友，似乎没有找到匹配项");
          dispose();
        }
      });
    } else {
  ...
  ctx.command("dd添加好友").action(async ({ session: session2 }) => {
    session2.send("请在游戏中点击皮肤目录,并返回上一级菜单,双击settings_ddnet.cfg,用笔记本或记事本打开,复制里面所有的内容粘贴过来\n输入:取消\n可以取消输入");
    const content = await session2.prompt();
    if (content === "取消" || content === null) {
      await session2.send("已取消输入");
      return;
    }
    const userid = session2.userId;
    ...
    return void session2.send("添加成功");
  });
  ctx.command("添加玩家").action(async ({ session: session2 }) => {
    await addplayer(ctx, { session: session2 });
    return;
  });
  ctx.command("points [...args:string]").alias("查分").action(async ({ session: session2, args }) => {
    if (args.length > 0) {
  ...
  ctx.command("删除玩家").action(async ({ session: session2 }) => {
    await deleteplayer(ctx, { session: session2 });
    return;
  });
  ctx.command("地图情况 [...args:string]").action(async ({ session: session2, args }) => {
    if (args.length > 0) {
      session2.send("请输入要查询的序列号\n1.简单图(Novice)\n2.中阶图(Moderate)\n3.高阶图(Brutal)\n4.疯狂图(Insane)\n5.传统图(Oldschool)\n6.古典图(DDmaX)\n7.分身图(Dummy)\n8.单人图(Solo)\n9.竞速图(Race)");
      const select = await session2.prompt(1e4);
      if (!select) return;
      else fetchPlayerData(args, select, session2);
    } else {
      return void session2.send("未输入玩家ID,请重新输入\n如 地图情况 我的ID");
    }
  });
  ctx.command("删除玩家").action(async ({ session: session2 }) => {
    await deleteplayer(ctx, { session: session2 });
  ...
  ctx.command("关注 [...args:string]").action(async ({ session: session2, args }) => {
    if (args.length > 0) {
